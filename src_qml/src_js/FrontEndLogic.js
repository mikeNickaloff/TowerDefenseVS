   ...
   // grid.setWalkableAt(col_2, row_2, p2_walkable);
      ...
      if (rv[2] != null) {
          if (targetObject == null) {
              console.log("Got request to generate null target path");
            game.board.clear_path_data(row_1, col_1);
            var q = 0;
            while (rv[q] != null) {
               game.board.add_path_data(parseInt(rv[q][0]),parseInt(rv[q][1]), row_1, col_1);
               q++;

           }
            game.board.next_exit();
          } else {

              console.log("Got request to generate targetted path");
